"""
    This module is the glue to tie together all the execution and consensus
    client bootstrapping phases. There is a high level doc in docs that
...
from .ETBConfig import ETBConfig, FilesConfig, ClientInstance, ClientInstanceCollectionConfig
from .UtilityWrappers import Eth2ValTools
...
class EthereumTestnetBootstrapper(object):
    ...
    def _pair_execution_clients(self, etb_config: ETBConfig, global_timeout: int):
        ...
        for el_client, rpc_request_result in perform_batched_request(node_info_rpc_request, el_clients_to_pair):
            if rpc_request_result.valid_response:
                ...
                err = rpc_request_result.last_seen_exception
                self.logger.error(
                    f"Failed to get enode from client: {el_client.name}. Exception: {err}"
                )
                # bail early as something has happened. Either bad config or no launch.
                raise err
        ...
        for el_peer, enode in enodes.items():
            ...
            for el_client in el_clients_to_pair:
                ...
                if el_client != el_peer:
                    self.logger.debug("adding peer {el_peer} to el_client {el_client}")
                    err = add_enode_rpc_request.perform_request(el_client)
                    if err is not None:
    ...
    def _write_validator_keystores(self, etb_config: ETBConfig):
        ...
        eth2_val_tools = Eth2ValTools(self.logger)
    ...
    def get_deposit_contract_deployment_block(self, etb_config: ETBConfig, global_timeout: int) -> tuple[str, int]:
        ...
        get_block_rpc_request = eth_getBlockByNumber("0x0", self.logger, timeout=global_timeout)
        err = get_block_rpc_request.perform_request(target_instance)
        if err is not None:
            self.logger.error(
                f"failed to get the contract deployment from client: {target_instance}. err: {err}"
            )
            raise err
