...
class EthereumTestnetBootstrapper(object):
    ...
    def clean(self):
        ...
        logging.info(f"Cleaning up the testnet directories: {files_config.testnet_root}")
    ...
    def create_keystores(self, config_path: Path):
        ...
        logging.warning("This will not init a testnet or bootstrap one. See docs for more info.")
    ...
    def _pair_execution_clients(self, etb_config: ETBConfig, global_timeout: int):
        ...
        for instance in client_instances:
            if admin_api_filter.search(",".join(instance.execution_config.http_apis)):
                ...
                logging.warning(f"Execution client for {instance.name} does not support the admin API.")
                logging.info(f"Skipping execution pairing for instance: {instance.name}")
        ...
        for el_client, rpc_future in perform_batched_request(rpc_request, el_clients_to_pair).items():
            ...
            if rpc_request.is_valid(result):
                ...
                logging.error(f"Failed to get enode from {el_client.name}, error: {result}")
                # bail early
                raise result

        logging.debug(f"Fetched the following enodes: {enodes} from the execution clients.")
    ...
    def get_deposit_contract_deployment_block(self, etb_config: ETBConfig, global_timeout: int) -> tuple[str, int]:
        ...
        logging.debug(f"Using instance: {target_instance.name} to get the contract deployment block.")
...
if __name__ == "__main__":
    ...
    create_logger(log_level=args.log_level, name="testnet_bootstrapper", log_to_file=True)
    ...
    if args.init_testnet:
        ...
        if args.config.split("/")[0] != "source":
            logging.debug("prepending source to the config path in order to map in the config file.")
